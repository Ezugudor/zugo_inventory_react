...
const showResponse = props => {
  const ppData = props.drivers.map((res, index) => {
    const {
      sn,
      id,
      avatar,
  ...
  ppDataS.rows = ppData;
  ppDataS.title = `<i class="${Style.tableTitleIcon} ion ion-ios-home"></i> Drivers`;
  ppDataS.newBtn = (
    <Red
      className={Style.btn}
      extStyle={Style.btn}
    ...
    >
      ...
      <span className={Style.btnText}> New Driver </span>
    </Red>
  );
