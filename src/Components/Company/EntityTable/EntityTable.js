...
const showResponse = props => {
  const ppData = dataStruct.rows.map((res, index) => {
    const { sn, id, name, address, phone, date } = res;
    const rowData = {
      sn,
      id,
      name,
  ...
  ppDataS.rows = ppData;
  ppDataS.title = `<i class="${Style.tableTitleIcon} ion ion-ios-home"></i> Company List`;
  ppDataS.newBtn = (
    <Red
      className={Style.btn}
      extStyle={Style.btn}
    ...
    >
      ...
      <span className={Style.btnText}> New Company </span>
    </Red>
  );
