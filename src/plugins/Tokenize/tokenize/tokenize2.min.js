!(function(t) {
  ...
  var e = function(o, i) {
      (this.control = !1),
        ...
        (this.options.tabIndex =
          -1 === this.options.tabIndex ? 0 : this.options.tabIndex),
        (this.options.sortable =
          1 !== this.options.tokensMaxItems && this.options.sortable),
        this.bind(),
  ...
  (e.VERSION = "1.0"),
    ...
    (e.prototype.init = function() {
      (this.id = this.guid()),
        ...
        this.element.attr("multiple") ||
          console.error(
            "Attribute multiple is missing, tokenize2 can be buggy !"
          ),
        (this.dropdown = void 0),
        (this.searchContainer = t('<li class="token-search" />')),
        (this.input = t('<input autocomplete="off" />')
          .on(
            "keydown",
            {},
            t.proxy(function(t) {
          ...
          .on(
            ...
            {},
            t.proxy(function() {
              this.options.tokensAllowCustom &&
                setTimeout(
                  t.proxy(function() {
        ...
        (this.tokensContainer = t(
          '<ul class="tokens-container form-control" />'
        )
          .addClass(this.element.attr("data-class"))
          .attr("tabindex", this.options.tabIndex)
          .append(this.searchContainer.append(this.input))),
        !1 !== this.options.placeholder &&
          ((this.placeholder = t('<li class="placeholder" />').html(
          ...
          this.element.on(
            ...
            t.proxy(function() {
              ...
              t("li.token", this.tokensContainer).length > 0 ||
              this.input.val().length > 0
                ? this.placeholder.hide()
                : this.placeholder.show();
            }, this)
          )),
        (this.container = t('<div class="tokenize" />').attr("id", this.id)),
        this.container.append(this.tokensContainer).insertAfter(this.element),
        this.container
          .focusin(
            t.proxy(function(e) {
              this.trigger("tokenize:select", [
                t(e.target)[0] === this.tokensContainer[0]
              ]);
            }, this)
          )
          .focusout(
        ...
        1 === this.options.tokensMaxItems && this.container.addClass("single"),
        this.options.sortable &&
          (void 0 !== t.ui
        ...
        this.element
          .on(
            ...
            t.proxy(function() {
              ...
              t("li.token", this.tokensContainer).length >=
    ...
    (e.prototype.reorder = function() {
      ...
      this.options.sortable &&
        t.each(
          this.tokensContainer.sortable("toArray", { attribute: "data-value" }),
          t.proxy(function(i, n) {
            (o = t('option[value="' + n + '"]', this.element)),
              void 0 === e ? o.prependTo(this.element) : e.after(o),
              (e = o);
          }, this)
        );
    }),
    (e.prototype.paste = function() {
      ...
      e.test(this.input.val()) &&
        t.each(
          this.input.val().split(e),
          t.proxy(function(t, e) {
    ...
    (e.prototype.tokenAdd = function(e, o, i) {
      if (
        ...
        (i = i || !1),
        this.resetInput(),
        void 0 === e || (!this.options.allowEmptyValues && "" === e))
      )
        return this.trigger("tokenize:tokens:error:empty"), this;
      if (
        this.options.tokensMaxItems > 0 &&
        t("li.token", this.tokensContainer).length >=
          this.options.tokensMaxItems
      )
        return this.trigger("tokenize:tokens:error:max"), this;
      if (
        t('li.token[data-value="' + e + '"]', this.tokensContainer).length > 0
      )
        return this.trigger("tokenize:tokens:error:duplicate", [e, o]), this;
      if (t('option[value="' + e + '"]', this.element).length)
        t('option[value="' + e + '"]', this.element)
        ...
        if (!this.options.tokensAllowCustom)
          return (
            this.trigger("tokenize:tokens:error:notokensAllowCustom"), this
          );
        this.element.append(
          t('<option selected data-type="custom" />')
            .val(e)
            .html(o)
        );
      }
    ...
    (e.prototype.tokenRemove = function(e) {
      ...
      return (
        "custom" === o.attr("data-type")
          ? o.remove()
          : o.removeAttr("selected").prop("selected", !1),
    ...
    (e.prototype.remap = function() {
      return (
        t("option:selected", this.element).each(
    ...
    (e.prototype.blur = function() {
      this.isDropdownOpen() && this.trigger("tokenize:dropdown:hide"),
        this.container.removeClass("focus"),
        this.resetPending(),
        this.tokensContainer.attr("tabindex") ||
        ...
        switch (e.keyCode) {
          case o:
            if (this.input.val().length < 1) {
              if (
                (e.preventDefault(),
                t("li.token.pending-delete", this.tokensContainer).length > 0)
              )
                this.trigger("tokenize:tokens:remove", [
                  t("li.token.pending-delete", this.tokensContainer)
                    .first()
                    .attr("data-value")
                ]);
              else {
    ...
    (e.prototype.keypress = function(t) {
      if ("keypress" !== t.type || this.element.is(":disabled"))
        t.preventDefault();
      else {
        var e = !1;
        Array.isArray(this.options.delimiter)
          ? this.options.delimiter.indexOf(String.fromCharCode(t.which)) >= 0 &&
            (e = !0)
          : String.fromCharCode(t.which) === this.options.delimiter && (e = !0),
          e && this.pressedDelimiter(t);
      }
    }),
    (e.prototype.pressedDelimiter = function(e) {
      this.resetPending(),
        this.isDropdownOpen() &&
        t("li.active", this.dropdown).length > 0 &&
        !1 === this.control
          ? (e.preventDefault(),
            t("li.active a", this.dropdown).trigger("mousedown"))
          : this.input.val().length > 0 &&
            (e.preventDefault(),
            this.trigger("tokenize:tokens:add", [this.input.val()]));
    }),
    (e.prototype.find = function(t) {
      if (t.length < this.options.searchMinLength)
        return this.trigger("tokenize:dropdown:hide"), !1;
      (this.lastSearchTerms = t),
        "select" === this.options.dataSource
          ? this.dataSourceLocal(t)
          : "function" == typeof this.options.dataSource
          ? this.options.dataSource(t, this)
          : this.dataSourceRemote(t);
    }),
    (e.prototype.dataSourceRemote = function(e) {
      this.debounce(
        t.proxy(function() {
          void 0 !== this.xhr && this.xhr.abort(),
            (this.xhr = t.ajax(this.options.dataSource, {
              data: { search: e },
              dataType: "json",
    ...
    (e.prototype.dataSourceLocal = function(e) {
      var o = this.transliteration(e),
        i = [],
        n = (this.options.searchFromStart ? "^" : "") + this.escapeRegex(o),
        s = new RegExp(n, "i"),
        r = this;
      t("option", this.element)
        .not(":selected, :disabled")
        .each(function() {
          s.test(r.transliteration(t(this).html())) &&
            i.push({ value: t(this).attr("value"), text: t(this).html() });
        }),
    ...
    (e.prototype.debounce = function(e, o) {
      ...
      void 0 !== this.debounceTimeout && clearTimeout(this.debounceTimeout),
        (this.debounceTimeout = setTimeout(n, o || 0));
    }),
    (e.prototype.dropdownShow = function() {
      this.isDropdownOpen() ||
        (t(".tokenize-dropdown").remove(),
        (this.dropdown = t(
    ...
    (e.prototype.calculatezindex = function() {
      var t = this.container,
        e = 0;
      if (
        (!isNaN(parseInt(t.css("z-index"))) &&
          parseInt(t.css("z-index")) > 0 &&
          (e = parseInt(t.css("z-index"))),
        e < 1)
      )
        for (; t.length; )
      ...
      e && e.length > 0
        ? (this.trigger("tokenize:dropdown:show"),
          ...
          t.each(
            e,
            t.proxy(function(e, o) {
              t("li.dropdown-item", this.dropdown).length <=
                this.options.dropdownMaxItems &&
                this.trigger("tokenize:dropdown:itemAdd", [o]);
            }, this)
          ),
          t("li.active", this.dropdown).length < 1 &&
            t("li:first", this.dropdown).addClass("active"),
          t("li.dropdown-item", this.dropdown).length < 1
            ? this.trigger("tokenize:dropdown:hide")
            : this.trigger("tokenize:dropdown:filled"))
        : this.options.displayNoResultsMessage
        ? (this.trigger("tokenize:dropdown:show"),
          this.trigger("tokenize:dropdown:clear"),
          this.dropdown
            .find(".dropdown-menu")
            .append(
              t('<li class="dropdown-item locked" />').html(
                this.options.noResultsMessageText.replace(
                  "%s",
                  this.input.val()
                )
              )
    ...
    (e.prototype.dropdownSelectionMove = function(e) {
      if (t("li.active", this.dropdown).length > 0)
        if (
          t("li.active", this.dropdown).is(
            "li:" + (e > 0 ? "last-child" : "first-child")
          )
        )
          t("li.active", this.dropdown).removeClass("active"),
            t(
              "li:" + (e > 0 ? "first-child" : "last-child"),
              this.dropdown
            ).addClass("active");
        else {
          var o = t("li.active", this.dropdown);
          o.removeClass("active"),
            e > 0 ? o.next().addClass("active") : o.prev().addClass("active");
        }
      else t("li:first", this.dropdown).addClass("active");
    }),
    (e.prototype.dropdownAddItem = function(e) {
      if (this.isDropdownOpen()) {
        var o = t('<li class="dropdown-item" />')
          .html(this.dropdownItemFormat(e))
          .on(
            "mouseover",
            t.proxy(function(e) {
              e.preventDefault(),
            ...
            t.proxy(function(e) {
              e.preventDefault(),
                ...
                this.trigger("tokenize:tokens:add", [
                  ...
                  t(e.target).attr("data-text"),
                  !0
                ]);
            }, this)
          );
        t(
          'li.token[data-value="' + o.find("a").attr("data-value") + '"]',
          this.tokensContainer
        ).length < 1 &&
          (this.dropdown.find(".dropdown-menu").append(o),
          this.trigger("tokenize:dropdown:itemAdded", [e]));
      }
    }),
    ...
    (e.prototype.dropdownItemFormat = function(e) {
      if (e.hasOwnProperty("text")) {
        ...
        return t("<a />")
          .html(o)
          .attr({ "data-value": e.value, "data-text": e.text });
      }
    }),
    (e.prototype.dropdownMove = function() {
      var t = this.tokensContainer.offset(),
        e = this.tokensContainer.outerHeight(),
    ...
    (e.prototype.clear = function() {
      return (
        t.each(
          t("li.token", this.tokensContainer),
          t.proxy(function(e, o) {
    ...
    (e.prototype.resetPending = function() {
      ...
      e.length > 0 &&
        (this.trigger("tokenize:tokens:cancelDelete", [e.attr("data-value")]),
        e.removeClass("pending-delete"));
    }),
      ...
      this.ctx ||
        ...
        (t =
          ...
          (e =
            ...
            (Math.ceil(
              parseFloat(this.tokensContainer.css("border-left-width"))
            ) +
              Math.ceil(
                parseFloat(this.tokensContainer.css("border-right-width")) +
                  Math.ceil(
                    parseFloat(this.tokensContainer.css("padding-left"))
                  ) +
                  Math.ceil(
                    parseFloat(this.tokensContainer.css("padding-right"))
                  )
              ))) && (t = e),
        this.searchContainer.width(t),
        this.ctx.restore();
    }),
    ...
    (e.prototype.toArray = function() {
      var e = [];
      return (
        t("option:selected", this.element).each(function() {
          e.push(t(this).val());
        }),
        e
      );
  ...
  (t.fn.tokenize2 = function(o) {
    ...
    (t.fn.tokenize2.Constructor = e),
    (t.fn.tokenize2.noConflict = function() {
